...
/**
 * @Route("/adminka/matkas/plemmatka/{plemmatka_id}/redaktors/departments", name="adminka.matkas.plemmatka.redaktors.departments")
...
//            'departments' => $plemmatka->getDepartments(),
    ...
    public function edit(PlemMatka $plemmatka, string $id, Request $request, Edit\Handler $handler): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $handler->handle($command);
                return $this->redirectToRoute('adminka.matkas.plemmatka.redaktors.departments.show', ['plemmatka_id' => $plemmatka->getId(), 'id' => $id]);
            } catch (\DomainException $e) {
                $this->errors->handle($e);
                $this->addFlash('error', $e->getMessage());
            }
        ...
        return $this->render('app/adminka/matkas/plemmatka/redaktors/departments/edit.html.twig', [
            'plemmatka' => $plemmatka,
