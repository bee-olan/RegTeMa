...
use App\Model\Adminka\UseCase\Sezons\Godas\Edit;
use App\Model\Adminka\UseCase\Sezons\Godas\Remove;
...
class GodaController extends AbstractController
{
    ...
    public function index(GodaFetcher $godas): Response
    {
        ...
        $godass = $godas->all();
//dd($godass );
        return $this->render('app/adminka/sezons/godas/index.html.twig', compact('godass'));
    }
    ...
    public function edit(Goda $goda, Request $request, Edit\Handler $handler): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $handler->handle($command);
                return $this->redirectToRoute('adminka.sezons.godas.show', ['id' => $goda->getId()]);
            } catch (\DomainException $e) {
                $this->logger->warning($e->getMessage(), ['exception' => $e]);
                $this->addFlash('error', $e->getMessage());
            }
        ...
        return $this->render('adminka/sezons/godas/edit.html.twig', [
            'godas' => $goda,
