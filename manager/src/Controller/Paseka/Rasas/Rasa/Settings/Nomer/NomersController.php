...
/**
 * @Route("/paseka/rasas/rasa/settings/linias/nomers/{linia_id}", name="paseka.rasas.rasa.settings.linias.nomers")
...
//dd( $linia->getId()->getValue());
     ...
     * @return Response
     */
    public function create(Linia $linia, 
                            NomerFetcher $nomers, 
                            Request $request, 
                            Create\Handler $handler): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                return $this->redirectToRoute('paseka.rasas.rasa.settings.linias.nomers', ['linia_id' => $linia->getId()]);
            } catch (\DomainException $e) {
                $this->errors->handle($e);
                $this->addFlash('error', $e->getMessage());
            }
        }
        return $this->render('app/paseka/rasas/rasa/settings/linias/nomers/create.html.twig', [
            'linia' => $linia,
    ...
    public function edit(Linia $linia, string $id, Request $request, Edit\Handler $handler): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
				$command->title = $linia->getTitle();
                $handler->handle($command);
                return $this->redirectToRoute('paseka.rasas.rasa.settings.linias.nomers.show', 
        ...
        return $this->render('app/paseka/rasas/rasa/settings/linias/nomers/edit.html.twig', [
            'linia' => $linia,
            'nomer' => $nomer,
            'form' => $form->createView(),
        ]);
    ...
    public function delete(Linia $linia, string $id, Request $request, Remove\Handler $handler): Response
    {
        ...
        return $this->redirectToRoute('paseka.rasas.rasa.settings.linias.nomers', 
