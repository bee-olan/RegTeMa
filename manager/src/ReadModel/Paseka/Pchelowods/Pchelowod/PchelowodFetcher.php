...
class PchelowodFetcher
{
    ...
    public function __construct(Connection $connection, EntityManagerInterface $em, PaginatorInterface $paginator)
    {
        ...
        $this->repository = $em->getRepository(Pchelowod::class);
        $this->paginator = $paginator;
    }

    public function find(string $id): ?Pchelowod
    {
    ...
    public function all(Filter $filter, int $page, int $size, string $sort, string $direction): PaginationInterface
    {
        $qb = $this->connection->createQueryBuilder()
            ...
            ->from('paseka_pchelowods_pchelowods', 'm')
            ->innerJoin('m', 'paseka_pchelowods_groups', 'g', 'm.group_id = g.id');
    ...
    public function exists(string $id): bool
    {
        return $this->connection->createQueryBuilder()
                ->select('COUNT (id)')
                ->from('paseka_pchelowods_pchelowods')
                ->where('id = :id')
                ->setParameter(':id', $id)
    ...
    public function activeGroupedList(): array
    {
        $stmt = $this->connection->createQueryBuilder()
            ...
            ->from('paseka_pchelowods_pchelowods', 'm')
            ->leftJoin('m', 'paseka_pchelowods_groups', 'g', 'g.id = m.group_id')
