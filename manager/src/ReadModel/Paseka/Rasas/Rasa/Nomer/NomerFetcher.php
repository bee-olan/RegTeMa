...
namespace App\ReadModel\Paseka\Rasas\Rasa\Nomer;
...
class NomerFetcher
{
    ...
    public function getMaxSortNomer(string $linia): int
    {
        return (int)$this->connection->createQueryBuilder()
            ...
            ->from('paseka_rasas_rasa_linia_nomers', 'n')
    ...
    public function listOfLinia(string $linia): array
    {
        $stmt = $this->connection->createQueryBuilder()
            ...
            ->from('paseka_rasas_rasa_linia_nomers')
            ->andWhere('linia_id = :linia')
            ->setParameter(':linia', $linia)
            ->orderBy('name')
            ->orderBy('name_star')
    ...
    public function allOfLinia(string $linia): array
    {
        $stmt = $this->connection->createQueryBuilder()
            ...
            ->from('paseka_rasas_rasa_linia_nomers', 'd')
            ->andWhere('linia_id = :linia')
            ->setParameter(':linia', $linia)
            //->innerJoin('d', 'rabota_rasas_linias_nomers_sparings', 's', 'd.sparing_id = s.id')
            ->orderBy('name')
            ->orderBy('name_star')
            ->orderBy('title')
            ->orderBy('sort_nomer')
